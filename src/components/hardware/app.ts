...
export function initHardwareApp(root: HTMLElement): void {
  ...
  // El catálogo es la parte pesada de la app. La página ya contiene la tabla
  // completa en HTML para el camino sin JS; el motor interactivo lo pide solo
  // cuando la persona empieza a usar el formulario.
    ...
    if (!catalogPromise) {
      catalogPromise = fetch('/hardware/catalog.json', { headers: { accept: 'application/json' } })
        .then(async (response) => {
          ...
          modelCatalog = models as ModelSpec[];
        })
        .catch(() => {
          // Si el asset está bloqueado, la tabla sin JS sigue disponible y no
          // se convierte un fallo opcional en un error de consola.
        })
        .finally(() => {
          catalogReady = true;
        });
