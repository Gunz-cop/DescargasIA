...
| 2026-08-24 | F8 | El catálogo de GPUs/modelos se sirve como asset estático (`src/pages/hardware/catalog.json.ts`, patrón de `search-index.json.ts`) y el motor interactivo lo pide recién al primer interactuar | El JS inicial de `/puedo-correr-ia` incluía los ~360 KB de `gpus.json`+`models.json` importados de forma estática en `app.ts` y `detect.ts` (bundle de 288 KB); F8 exige que los datos vayan diferidos, no en el HTML/JS inicial. Tras el cambio el bundle inicial baja a ~25 KB y el catálogo se sirve cacheable aparte |
| 2026-08-24 | F8 | El enlace "desde las fichas de los runtimes" que pide F8 se implementó en `src/pages/[lang]/[slug].astro` (una `<aside>` condicionada a `tool.slug` en `['ollama','lm-studio','jan']`), no solo editando los JSON de esas fichas | El esquema de `editorialSections`/`faq`/etc. en `src/content.config.ts` no soporta enlaces (el body se renderiza como texto plano, sin markdown ni HTML), así que un enlace real solo podía salir de la plantilla. Los JSON de las tres fichas no se modificaron para este fin |
| 2026-08-24 | F8 | La auditoría de contenido de `descargasia-ficha-auditoria` no aplica directamente a `/puedo-correr-ia` (no es una ficha del content collection `tools`); se revisó a mano contra los mismos criterios (sin claims inventados, tok/s siempre rotulado como estimación, metodología publicada) y se corrió el script de métricas sobre `ollama`/`lm-studio`/`jan`, que ahora enlazan hacia la app | `0 bloqueantes` en las tres fichas; se verificaron manualmente contra la fuente las 5 afirmaciones numéricas del insight de `ollama` (175.000 servidores, 130 países, ~30% China, 48% con tool-calling, la cita textual sobre el riesgo) y las 5 coinciden con el artículo citado |
| 2026-08-24 | F8 | El gap de infraestructura ya reportado en el issue #13 (no existe workflow de Branch Preview de Cloudflare; los criterios `[manual]` de Lighthouse/lector de pantalla no se pudieron verificar sobre la URL que describe la spec) sigue vigente | No se duplicó el comentario en el issue; se dejó constancia acá para que quien revise sepa que dos sesiones independientes de F8 llegaron al mismo hallazgo |
