...
- [ ] **Comprobador de guías** (recibe el directorio por argumento, para poder ejecutarse igual sobre una fixture): `node -e "const fs=require('fs');const d=process.argv[1];const g=fs.readdirSync(d).filter(f=>f.endsWith('.md'));process.exit(g.length===1&&g[0]==='descargar-chatgpt-para-windows.md'?0:1)" src/content/guides` sale 0 — no se ha añadido ninguna guía nueva mientras el bloqueo está vigente.
- [ ] `[manual]` **Prueba con dato inválido del comprobador de guías**, sobre una fixture real y sin tocar el repositorio: 1. `node -e "const fs=require('fs'),os=require('os'),p=require('path');const d=fs.mkdtempSync(p.join(os.tmpdir(),'f3es-guias-'));fs.writeFileSync(p.join(d,'descargar-chatgpt-para-windows.md'),'fixture');fs.writeFileSync(p.join(d,'ia-sin-conexion-en-tu-pc.md'),'fixture');console.log(d)"` crea en el directorio temporal una copia del directorio de guías con una guía nueva e imprime su ruta; 2. ejecuta **el mismo comando del comprobador de guías** con esa ruta como argumento; 3. resultado esperado: sale 1, porque el comprobador lee el directorio y encuentra dos guías; 4. borra la fixture con `node -e "require('fs').rmSync(process.argv[1],{recursive:true,force:true})" <ruta impresa>`.
