...
- [ ] `node -e "const fs=require('fs');const t=JSON.stringify(JSON.parse(fs.readFileSync('src/content/tools/es/mistral-vibe.json','utf8'))).toLowerCase();if(!t.includes('le chat'))process.exit(1);const enc=(fs.readFileSync('src/content/tools/es/mistral-vibe.json','utf8'));const j=JSON.parse(enc);if(!(j.shortDescription+' '+(j.editorialSummary||'')).toLowerCase().includes('le chat'))process.exit(1)"` sale 0 — el nombre antiguo aparece en la respuesta above the fold, no solo enterrado en el cuerpo.
- [ ] `node -e "const j={shortDescription:'Asistente de Mistral',editorialSummary:'Resumen sin el nombre antiguo'};process.exit((j.shortDescription+' '+j.editorialSummary).toLowerCase().includes('le chat')?1:0)"` sale 0 — **prueba con dato inválido**: la regla anterior rechaza un above the fold que omite el nombre antiguo.
