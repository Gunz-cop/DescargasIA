...
- [ ] `node -e "const fs=require('fs');const prohibidos=['uptodown','malavida','onworks','softonic','artistapirata','mega.nz'];for(const s of ['ollama','cursor']){const t=fs.readFileSync('src/content/tools/es/'+s+'.json','utf8').toLowerCase();for(const d of prohibidos)if(t.includes(d))process.exit(1);}"` sale 0 — el copy no enlaza ni nombra como destino ninguno de los portales observados en la SERP.
- [ ] `node -e "const prohibidos=['uptodown','malavida','onworks','softonic','artistapirata','mega.nz'];const muestra='descarga desde uptodown.com';process.exit(prohibidos.some(d=>muestra.includes(d))?0:1)"` sale 0 — **prueba con dato inválido**: la regla anterior detecta un portal prohibido cuando existe.
